ItaPoly = function( _width, _height ){
  ...
  var gl = it.gl;
  it.gl.floatExtension = gl.getExtension( 'OES_texture_float' );
...
ItaPoly.prototype.createProgram = function( _frag, _callback ){
    ...
    it.error = '';
		it.program = p;
    it.program.locations = {};

    it.program.textures = [];
    it.program.textureNumbers = [];
    it.program.textureCount = 0;
...
ItaPoly.prototype.setFloat = function( _name, _param ){
  ...
  if( !it.program.locations[ _name ] ){ it.program.locations[ _name ] = it.gl.getUniformLocation( it.program, _name ); }
  it.gl.uniform1f( it.program.locations[ _name ], _param );
...
ItaPoly.prototype.setVec2 = function( _name, _param ){
  ...
  if( !it.program.locations[ _name ] ){ it.program.locations[ _name ] = it.gl.getUniformLocation( it.program, _name ); }
  it.gl.uniform2fv( it.program.locations[ _name ], _param );
...
ItaPoly.prototype.setVec3 = function( _name, _param ){
  ...
  if( !it.program.locations[ _name ] ){ it.program.locations[ _name ] = it.gl.getUniformLocation( it.program, _name ); }
  it.gl.uniform3fv( it.program.locations[ _name ], _param );
...
ItaPoly.prototype.setVec4 = function( _name, _param ){
  ...
  if( !it.program.locations[ _name ] ){ it.program.locations[ _name ] = it.gl.getUniformLocation( it.program, _name ); }
  it.gl.uniform4fv( it.program.locations[ _name ], _param );
...
ItaPoly.prototype.setInt = function( _name, _param ){
  ...
  if( !it.program.locations[ _name ] ){ it.program.locations[ _name ] = it.gl.getUniformLocation( it.program, _name ); }
  it.gl.uniform1i( it.program.locations[ _name ], _param );
...
ItaPoly.prototype.prepareTexture = function( _name ){
  ...
  if( it.program.textureNumbers[ _name ] !== 0 && !it.program.textureNumbers[ _name ] ){

    var i = it.program.textureCount;

    gl.activeTexture( gl.TEXTURE0 + i );
    it.program.textures[ i ] = gl.createTexture();
    gl.bindTexture( gl.TEXTURE_2D, it.program.textures[ i ] );
  	gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST );
  	gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST );
  	gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE );
  	gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE );

    it.program.textureNumbers[ _name ] = i;
    it.program.locations[ i ] = gl.getUniformLocation( it.program, _name );

    it.program.textureCount += 1;
...
ItaPoly.prototype.setSampler2D = function( _name, _param ){
  ...
  it.prepareTexture( _name );
	...
	gl.bindTexture( gl.TEXTURE_2D, it.program.textures[ it.program.textureNumbers[ _name ] ] );
...
ItaPoly.prototype.setSampler2DFromFloatArray = function( _name, _width, _height, _array ){
...
ItaPoly.prototype.setSize = function( _width, _height ){
  ...
  it.gl.viewport( 0, 0, it.width, it.height );
...
ItaPoly.prototype.update = function(){
  ...
  for( var i=0; i<it.program.textures.length; i++ ){
    gl.bindTexture( gl.TEXTURE_2D, it.program.textures[ i ] );
    gl.uniform1i( it.program.locations[ i ], i );
  }
...
ItaPoly.prototype.readPixels = function( _x, _y, _w, _h ){
